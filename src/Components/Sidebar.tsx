...
function Sidebar() {
  ...
  return (
    <div className={`max-h-screen flex flex-col space-y-2 md:space-y-4 p-2 md:p-0`}>
      <div className={`bg-back rounded-xl font-mono transition-all ease-in-out duration-500 overflow-hidden flex-1 min-h-0 ${ToggleSidebar ? 'w-64 sm:w-72 md:w-64' : 'w-14 sm:w-16'
        }`}>
        <div className='p-2 md:p-4 h-full flex flex-col'>
          ...
          <button className={`cursor-pointer w-full flex items-center gap-2 mt-2 md:mt-0 ${ToggleSidebar
            ? 'border border-[#353535] p-2 rounded-lg hover:bg-WhenHover justify-center text-sm md:text-base'
      ...
      <div className={`bg-back rounded-xl transition-all ease-in-out duration-500 overflow-hidden flex-shrink-0 ${ToggleSidebar ? 'h-16 w-64 sm:w-72 md:w-64' : 'h-24 sm:h-32 md:h-32 w-14 sm:w-16'
        }`}>
        <div className={`h-full flex items-center transition-all duration-300 ${ToggleSidebar ? 'justify-center gap-3 md:gap-6 px-2 md:px-4' : 'flex-col justify-center gap-2 md:gap-4'
          ...
          <button
            ...
            className={`${ToggleSidebar
              ? 'text-xs md:text-sm font-mono border border-[#353535] rounded-xl cursor-pointer hover:bg-WhenHover transition-all duration-300 px-8 md:px-16 py-1.5 md:py-2 opacity-100 translate-x-0'
              : 'hidden'
              }`}
          >
            Login
          </button>
          <button className={`${ToggleSidebar ? 'hidden' : 'p-2 md:p-4 border rounded-full border-[#353535]'
            }`}></button>
